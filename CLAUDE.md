...
- **Atoms** = Semantic HTML elements (NO classes) - style `<button>`, not `.btn`
- **Molecules** = Component classes with container queries
- **Organisms** = Complex compositions

### Naming Conventions
- **Tokens:** `--p-{category}-{variant}` for primitives, `--{category}-{role}` for semantic
- **Components:** `.card`, `.card--featured` (flat, no BEM child selectors)
- **Layout utilities:** `.stack`, `.cluster`, `.grid`, `.center`

### JavaScript: Native-First
- "If a browser can do it natively with HTML or CSS, delete the JavaScript"
...
### CSS Organization
- **Single file preferred** - keep all CSS in `dist/css/html.style.css`
...
- Add JavaScript for things CSS can handle
