...
Beyond colour the primitive tier also covers motion (`--p-duration-*`,
`--p-ease-*`), border widths, and layering, with semantic counterparts
(`--motion-duration`, `--border-width`, `--focus-ring-*`, `--p-layer-*`). Never
hardcode a duration, focus ring, or border width — route it through a token, or
the value cannot be themed and cannot respond to user preferences.

### Component Settings Are Custom Properties

Every configurable thing a component exposes is `--hs-<element>-<setting>`, with
a default falling back to a semantic token:

```css
background: var(--hs-toggle-track-color, var(--color-border-emphasis));
```

The component follows the theme with no configuration and can be overridden
without any. **This is the only route into a shadow component** - `::part()`
restyles the elements a component exposes but loses to sizes declared inside the
shadow root. Derive dependent values rather than hardcoding them, so resizing one
thing moves what depends on it.
...
- **Every shadow component restates the reset it needs.** `box-sizing` and anything else declared as a plain rule stops at the shadow boundary. Omitting `box-sizing: border-box` silently resizes the component and surfaces as a layout shift on upgrade.
- **Reduced motion is the exception, and the reason is worth knowing.** Custom properties DO inherit through a shadow boundary, so the framework collapses `--motion-duration` on `:root` under `prefers-reduced-motion` and that reaches inside components. Build transitions from the token and no per-component media query is needed. The same trick works for anything else expressible as a token.
...
- A `custom-elements.json` manifest is generated by `build.js` from the shipped modules and declared through the `customElements` field in `package.json`. This is what makes the AI-friendly claim concrete rather than aspirational. Document a component's public surface with JSDoc tags on the CLASS (`@element`, `@attr`, `@slot`, `@fires`, `@csspart`, `@cssprop`) — tags in a module header comment are not picked up, and slots, parts and custom properties cannot be inferred from code.
