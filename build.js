...
function generateTypes() {
  ...
  /**
   * `JSX.IntrinsicElements`, so `<hs-menu label="Actions">` type-checks in a
   * `.tsx` file with no cast. A plain object type per tag, not
   * `React.DetailedHTMLProps` - importing React's types here would put every
   * consumer of this file on the hook for `@types/react`, including the
   * ones with no JSX at all. The index signature is the trade for that:
   * standard DOM/React props this generator does not enumerate (`onClick`,
   * `className`, `style`, ...) pass through loosely rather than erroring.
   */
  const jsxIntrinsics = elements
    .map((element) => {
      const attrs = (element.attributes ?? []).filter((a) => a.name);
      const props = attrs
        .map(
          (attr) =>
            comment(attr.description, '      ') +
            `      ${key(attr.name)}?: ${typeText(attr.type?.text, 'string')};\n`
        )
        .join('');
      return (
        `    ${key(element.tagName)}: {\n` +
